	...
	for($x=0; $x<count($plcs[groups]); $x++) {
		...
		if($grp[service_sum] == 0) {
			...
			$bar = "<div id='sys_health_base1' class='progress progress-success' style='width: 80px;'>
							...
							<div data-rel='tooltip' title='Infos/Downtimes:" . $grp[prozent_float][downtimes_and_infos] . "%' id='sys_health_progress3' class='bar' style='float:left; width: " . $grp[prozent_float][downtimes_and_infos] . "%;background-image: -webkit-linear-gradient(top, #A8A8A8, #CFCFCF);'></div>
