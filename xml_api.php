...
function xml_bartlby_get_service($args) {
	global $btl;
	return bartlby_get_service($btl->RES, $args);
}
function xml_bartlby_get_service_with_id($args) {
	global $btl;
	$idx=$btl->findSHMPlace($args);
	return bartlby_get_service($btl->RES, $idx);
}


function xml_bartlby_get_worker($args) {
	global $btl;
	return bartlby_get_worker($btl->RES, $args);	
}
function xml_bartlby_get_info($args) {
	global $btl;
	return bartlby_get_info($btl->RES );
}
function xml_bartlby_version($args) {
	global $btl;
	return bartlby_version($btl->RES);	
}
...
function xml_bartlby_lib_info($args) {
	...
	return bartlby_lib_info($btl->RES);
}
function xml_bartlby_add_server($args) {
	...
	$server=bartlby_add_server($btl->RES, $args[0], $args[1], $args[2], $args[3]);
	$add_service=bartlby_add_service($btl->RES, $server, "INIT", "Initial Check", "-h", 0, 0,24,0,59,2000,1,"",200, 20, 0);
	return $server;
}
function xml_bartlby_delete_server($args) {
	global $btl;
	return bartlby_delete_server($btl->RES, $args);	
}
function xml_bartlby_modify_server($args) {
	global $btl;
	//SERVERID NAME IP PORT ICON
	
	$mod_server=bartlby_modify_server($btl->RES, $args[0], $args[1], $args[2], $args[3], $args[4]);
	return $mod_server;	
}

function xml_bartlby_get_server_by_id($args) {
	global $btl;
	return bartlby_get_server_by_id($btl->RES, $args);	
}
...
function xml_bartlby_add_service($args) {
	...
	$ads=bartlby_add_service($btl->RES, $args[0], $args[1],$args[2],$args[3],1, $args[4], $args[5], $args[6], $args[7],$args[8],$args[9],$args[10],$args[11],$args[12],$args[13],$args[14],$args[15],$args[16],$args[17],$args[18],$args[19]);	
	return $ads;
}
function xml_bartlby_delete_service($args) {
	global $btl;
	return bartlby_delete_service($btl->RES, $args);	
}
...
function xml_bartlby_modify_service($args) {
	...
	$ads=bartlby_modify_service($btl->RES, $args[0] , $args[1], $args[2],$args[3],$args[4],1, $args[5], $args[6], $args[7], $args[8],$args[9],$args[10],$args[11], $args[12], $args[13], $args[14]);
	return $ads;
}
function xml_bartlby_get_service_by_id($args) {
	global	$btl;
	return bartlby_get_service_by_id($btl->RES, $args);
}
function xml_bartlby_add_worker($args) {
	global $btl;
	//MAIL, ICQ, SVC String, notify string, ACTIVE?, NAME, PASSWORD, TRIGGER string
	$add=bartlby_add_worker($btl->RES, $args[0], $args[1], $args[2], $args[3], $args[4], $args[5], $args[6], $args[7]);
	return $add;
}

function xml_bartlby_delete_worker($args) {
	global $btl;
	return bartlby_delete_worker($btl->RES, $args);
		
}

function xml_bartlby_modify_worker($args) {
	global $btl;	
	$mod=bartlby_add_worker($btl->RES, $args[0], $args[1], $args[2], $args[3], $args[4], $args[5], $args[6], $args[7], $args[8]);
	return $mod;
}
function xml_bartlby_get_worker_by_id($args) {
	global $btl;
	return bartlby_get_worker_by_id($btl->RES, $args);	
}
function xml_bartlby_add_downtime($args) {
	global $btl;
	//TIMESTAMP from, TIMESTAMP To, DT Type, NOTICE, service/server - id
	return bartlby_add_downtime($btl->RES, $args[0], $args[1], $args[2], $args[3], $args[4]);	
}
function xml_bartlby_downtime_map($args) {
	global $btl;
	return bartlby_downtime_map($btl->RES);	
}
function xml_bartlby_modify_downtime($args) {
	global $btl;
	//TIMESTAMP from, TIMESTAMP To, DT Type, NOTICE, service/server - id, DTid
	return bartlby_add_downtime($btl->RES, $args[0], $args[1], $args[2], $args[3], $args[4], $args[5]);	
		
}
function xml_bartlby_delete_downtime($args) {
	global $btl;
	return bartlby_delete_downtime($btl->RES, $args);	
}

function xml_bartlby_toggle_service_notify($args) {
	global $btl;
	$idx=$btl->findSHMPlace($args);
	$cur=bartlby_toggle_service_notify($btl->RES, $idx);
	return $cur;
}
...
function xml_bartlby_toggle_service_active($args) {
	global $btl;
	$idx=$btl->findSHMPlace($args);
	$cur=bartlby_toggle_service_active($btl->RES, $idx);	
	return $cur;
}

function xml_bartlby_toggle_sirene($args) {
	global $btl;
	return bartlby_toggle_sirene($btl->RES);	
}
function xml_bartlby_reload($args) {
	global $btl;
	return bartlby_reload($btl->RES);	
}
function xml_bartlby_shm_destroy($args) {
	global $btl;
	return bartlby_shm_destroy($btl->RES);	
}
function xml_bartlby_ack_problem($args) {
	global $btl;
	$idx=$btl->findSHMPlace($args);
	$r=bartlby_ack_problem($btl->RES, $idx);
	return $r;
}
function xml_bartlby_set_passive($args) {
	global $btl;
	$idx=$btl->findSHMPlace($args[0]);
	$r=bartlby_set_passive($btl->RES, $idx, $args[1], $args[2]);
	return $r;
}
