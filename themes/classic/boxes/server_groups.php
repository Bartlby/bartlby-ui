	...
	for($x=0; $x<count($plcs[groups]); $x++) {
		...
		if($grp[service_sum] == 0) {
			...
			$bar = "<div id='sys_health_base1' class='progress progress-success' style='width: 80px;'>
							...
							<div data-perc='" . $grp[prozent_float][downtimes_and_infos] . "'  data-rel='tooltip' title='Infos/Downtime: " . $grp[prozent_float][downtimes_and_infos] . "%' id='sys_health_progress3' class='bar' style='float:left; width: " . $grp[prozent_float][downtimes_and_infos] . "%;background-image: -webkit-linear-gradient(top,  #e0e0e0, #e8e8e8);'></div>
